...
if (collectionConfig.navigationStyle !== 'custom' || !collectionSquareTemplate.includes('collection_detail_bg.webp') || !collectionSquareTemplate.includes('class="detail-sheet surface"')) fail('收藏集详情必须使用 APP 蓝色头部和圆角文章区')
...
if (!columnSource.includes('loadDetailArticles(false)') || !columnTemplate.includes('column_default_cover.webp') || !columnTemplate.includes('class="article-actions"')) fail('专栏详情缺少 APP 头部、文章卡片或上拉分页')
