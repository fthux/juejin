...
if (postTemplate.includes('bindtap="addComment"') || postSource.includes('editable: true') || postSource.includes('session.addComment')) fail('文章详情不得保留评论输入或提交入口')
if (!postTemplate.includes('comment-entry-hidden')) fail('文章详情评论入口必须保留布局占位并隐藏')
...
const courseReaderSource = read('features/courseReader/courseReader.js')
const courseReaderTemplate = read('features/courseReader/courseReader.wxml')
const courseReaderStyles = read('features/courseReader/courseReader.wxss')
...
if (!pinDetailTemplate.includes('comment-input-hidden') || pinDetailTemplate.includes('class="comment-input" bindtap=')) fail('沸点详情不得保留评论输入点击入口')
...
if (!findTemplate.includes('class="pin-comment-box" aria-hidden="true"') || !/\.pin-comment-box\s*\{[^}]*visibility:\s*hidden;/s.test(findStyles)) fail('发现页精选沸点评论框必须隐藏且保留卡片占位')
if (courseReaderTemplate.includes('bindtap="showComments"') || courseReaderSource.includes('showComments()') || !courseReaderTemplate.includes('toolbar-item-hidden') || !courseReaderStyles.includes('.toolbar-item-hidden')) fail('小册阅读页不得保留评论入口')
